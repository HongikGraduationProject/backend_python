PROMPT = {
    ...
    만약 해당 영상이 특정 장소(카페, 식당, 관광지 등)를 다루고 있고, transcript나 description에 장소에 대한 주소가 포함되어 있는 경우 주소를 "address" 필드에 작성해줘. 주소를 나타낼 수 없다면 ""와 같이 빈 문자열을 채워넣어
    또한 title 필드가 공백인 경우 transcript나 description를 참고해서 적절한 무조건 제목을 지어줘. 절대 title 필드가 공백이면 안돼. 이미 내가 준 json 객체의 title 필드의 값이 존재한다면 해당 값을 그대로 적어줘
    단 여기서, 구독자에게 "좋아요" 버튼 누르기를 요청하는 등, 영상의 주제와 관련없는 내용은 요약에서 제외해.
    그리고 해당 영상의 카테고리를 아래의 10개 중 적절히 골라 채워줘.
    과학/기술(자동차, 로봇, 코딩 등등), 패션/뷰티(옷,화장품 등등), 요리, 리빙(생활), 건강, 여행, 예술(음악, 미술, 드라마, 영화), 뉴스, 엔터테인먼트(게임, 예능 등등), 기타
    요약한 후 { "summary" : "요약된 내용", "keywords" : ["키워드1","키워드2","키워드3"], "address" : "주소", "title" : "제목", "category" : "카테고리"}와 같은 형태로 응답해. 
    """,

    "SHORTS_SUMMARIZE_KOR_TEST": """
    지금부터 내가 주는 json 객체는 유튜브의 숏츠나 인스타의 릴스에 대한 정보를 담고있어. "title"은 영상의 제목을, "description"은 영상에 대한 설명, "transcript"는 영상의 대본을 의미해.
    영상에 대해 Topic-based summarization 을 할 거야. 
    주어진 객체들을 기반으로 영상의 구체적인 토픽과 테마를 추출해. 영상의 모든 중요한 정보를 요약해. 단, 요약할 때 "transcript" 를 가장 많이 참고해.
    내용 요약은 각 문장을 단문의 형태로 (1) (2) (3) (4)...와 같이 번호를 붙여 정리해. 최대 5줄 이내로 요약해줘.
    또한 아래의 요구사항들을 지켜서 요약해줘
    1. 만약 해당 영상이 특정 장소 (카페, 식당, 관광지 등)를 다루고 있고, transcript나 description에 장소에 대한 주소가 포함되어 있는 경우 주소를 "address" 필드에 작성해줘. 주소를 나타낼 수 없다면 ""와 같이 빈 문자열을 채워넣어. 주소가 없다면 해당 장소의 이름을 넣어도 좋아.
    2. 구독자에게 "좋아요" 버튼 누르기를 요청하는 등, 영상의 주제와 관련 없는 내용은 요약 과정에서 제외해.
    3. 내가 준 JSON 객체에서 "title" 필드가 공백인 경우, transcript나 description를 참고하여 적절한 제목을 작성해서 무조건 채워넣고, 공백이 아니라면 바꾸지 말고 그대로 채워넣어.
    4. 요약한 후 { "summary" : "요약된 내용", "keywords" : ["키워드1","키워드2","키워드3"], "address" : "주소", "title" : "제목"} 과 같은 형태로 응답해. 
    """
}
